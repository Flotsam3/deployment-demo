...
const handleTransactions = (state, action) => {
  ...
  switch (action.type) {
    ...
    case "updateAmount":
      return {amount: +action.payload.balance, transactions: [...reverseTransactions, {transaction:action.payload.transaction, value:action.payload.amount, date:getDate()}].reverse()}
    case "payIn": 
      if (action.payload === "" || isNaN(action.payload)) return state;
      return {amount: +state.amount + +action.payload, transactions: [...reverseTransactions, {transaction:"Einzahlung", value:action.payload, date:getDate()}].reverse()}
    case "chargeOff": 
      if (+state.amount - +action.payload < 0 || action.payload === "" || isNaN(action.payload)) return state;
      return {amount: +state.amount - +action.payload, transactions: [...reverseTransactions, {transaction:"Abbuchung", value:action.payload, date:getDate()}].reverse()}
    default:
      break;
  }
}
